...
# Default values for CZECHITAS_USER and CZECHITAS_EMAIL
...
while IFS=';' read -r WP_SERVER WP_TITLE WP_ADMIN_USER WP_ADMIN_EMAIL; do
  ...
  # Check if the environment variable for the admin password exists
  ADMIN_PASSWORD_VAR=$(echo "${WP_SITE}_PASSWORD" | tr '[:lower:]' '[:upper:]')
  if [ -z "${!ADMIN_PASSWORD_VAR}" ]; then
    echo "Error: Missing password for $WP_SITE (environment variable $ADMIN_PASSWORD_VAR)"
    continue
  fi
  WP_ADMIN_PASSWORD="${!ADMIN_PASSWORD_VAR}"
  ...
  # Check if the site contains index.html
  if [ -f "$SITE_PATH/index.html" ]; then
    rm "$SITE_PATH/index.html"
  ...
  if ! wp core is-installed --path="$SITE_PATH"; then
    ...
    wp core install --path="$SITE_PATH" --url="https://$WP_SERVER" --title="$WP_TITLE" --admin_user="$WP_ADMIN_USER" --admin_password="$WP_ADMIN_PASSWORD" --admin_email="$WP_ADMIN_EMAIL" --allow-root
    
    # Create the user for Czechitas
    wp user create --path="$SITE_PATH" "$CZECHITAS_USER" "$CZECHITAS_EMAIL" --role=administrator --user_pass="$WP_ADMIN_PASSWORD" --allow-root
